function paramValue=ReadPVParamRECO(fidPath,paramName,TwoDSeqPath)
%function paramValue=ReadPVParam(fidPath,paramName,TwoDSeqPath)
%
%read a parameter value from the parameter files (reco, visu_pars).
%
...
% if(isempty(paramName) || isempty(fidPath))
    ...
    %path to the reco and visu_pars files
    [dirPath]=fileparts(fidPath);
    recoPath=[dirPath,filesep,'reco'];
    visu_parsPath=[dirPath,filesep,'visu_pars'];
    ...
    %read the visu_pars file
    fid=fopen(visu_parsPath,'r');
    if(fid<0)
