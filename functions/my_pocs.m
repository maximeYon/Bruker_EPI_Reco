function [im, kspFull] = my_pocs( ksp, iter, watchProgress, ksp_phase )
...
%
%   ksp_phase:  kspace data used for phase determination, allowing constant
%   (optional)  phase in multiple repetion reconstruction
%     
% === Output ===
%
...
%           reconstructed data
% * changed by Maxime Yon to increase robustness (handle of sparse 0 pixel)
%
...
% =========================================================================
    ...
    %
        ...
        if ~exist('ksp_phase','var') || isempty(iter)
            ksp_phase = ksp;
        end
        ...
        if Ndim == 2    % Ny x Nx
            ...
            ksp_phase = reshape( ksp_phase, [1 size(ksp_phase)] );
    ...
    % Fill sparse zero voxel mod Maxime 2022
    if numel(size(smplPtrn))==2
        phase_vect = logical(sum(smplPtrn,1)>1);
        smplPtrn = repmat(phase_vect,size(smplPtrn,1),1);
%     smplPtrn = medfilt2(smplPtrn,'symmetric');
    end
    if numel(size(smplPtrn))==3
    smplPtrn = medfilt3(smplPtrn,'symmetric');
    end
    ...
    %
        if nnz(smplPtrn) == numel(smplPtrn)     % only the sampled data were passed / |N|umber of |N|on |Z|ero elements
            ...
            [ ksp_phase, ~, ~, ~, ~ ] = zerofillPFdim( ksp_phase, wasAddedCoilDim );
    ...
    if isLower
        ...
        ksp_phase           = ksp_phase(subs{:});       % !ecaps-k si sihT
    ...
    kspLowRes = bsxfun( @times, filter, ksp_phase);
