function [ data , addInfo] = my_readBrukerRaw_RGdata(Acqp, varargin)
...
%----------------------------------------------------------------
    ...
    if isNR_specified     
        ...
        if(single_bool)
            fidFile=( fread(fileID, [ACQ_size(1,1), inf], [format, '=>single'], 0, endian) );
        else
            fidFile=fread(fileID, [ACQ_size(1,1), inf], format, 0, endian);
        end    
    end
...
%         fidFile=reshape(fidFile,[size(fidFile,1), ACQ_size(1) ,numSelectedReceivers]); 
%            
%         % resort dimensions
...
fidFile=reshape(fidFile,[ACQ_size(1,1)./2,2, numSelectedReceivers, size(fidFile,2)]);
